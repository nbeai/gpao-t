...
## 2026-07-14 Closeout Addendum

아래의 2026-07-13 기준 경로와 중간 수치는 역사 기록으로 유지하되, 현재
종료 판단에는 다음 기준이 우선한다.

- canonical development source: `/Users/jyp/Developer/gpao-t`
- compatibility source snapshot: `~/.gpao-t/current/compatibility/gpao-t`
- canonical live runtime: `~/.gpao-t`
- canonical archive:
  `/Users/jyp/Developer/gpao-t/.gpao-t/releases/gpao-t-0.1.0.zip`
- archive SHA-256:
...
- human QA: `2 / 2`
- internal verification: human QA `2 / 2` recorded; source seal remains per section 3.2
- public release: 별도 권한 게이트

- 문서 상태: `LOCKED WORK ORDER`
- 기준 시각: 2026-07-13 KST
- 제품: `nBeAI. GPAO-T`
- 작업 대상: 독립 GPAO-T 소스, 프로덕션 패키지, macOS 라이브 런타임, 사용자 화면, 내부 검증 자료
- 최종 종료점: **내부 프로덕션 패키지와 동일한 산출물이 실제 사용자 PC에서 독립 GPAO-T로 정상 라이브되고, 설치·실행·복구 경로가 내부 검증된 상태**
...
- compatibility source snapshot:
  `~/.gpao-t/current/compatibility/gpao-t`
- 설치 후 라이브 제품 상태:
  `~/.gpao-t`

순정 비교 기준은 다음 하나만 사용한다.

- `/Users/jyp/Developer/_references/openclaw-pure-2026.6.11`

GPAO-T compatibility source는 제품 배포 기준이며 순정 OpenClaw 비교
기준으로 사용하지 않는다.
...
- 2026-07-13 10:15 KST 기준 당시 canonical 프로덕션 패키지 archive SHA-256은 `f3ba7c545c1d0fd4e64260253b47af0a388b5fee00f069f757ba7255e1313ad7`이다.
...
5. **내부 프로덕션 패키지 ZIP과 실제 라이브 설치에 사용하는 ZIP은 바이트 단위로 같은 파일이어야 한다.**
...
10. Telegram 실제 외부 발송은 내부 프로덕션 검증 완료 조건에 포함하지 않는다. 연결 상태와 전용 세션 경계까지만 검증한다.
...
현재 소스와 동일한 단 하나의 GPAO-T 프로덕션 패키지 ZIP을 만든다.
...
- output:
  `/Users/jyp/Developer/gpao-t/.gpao-t/releases/gpao-t-0.1.0`
- archive:
  `/Users/jyp/Developer/gpao-t/.gpao-t/releases/gpao-t-0.1.0.zip`
...
- 기존 handoff 인덱스의 과거 archive를 현재 프로덕션 패키지로 사용하지 않는다.
...
## 13. 공정 8: 전체 테스트·소스·내부 프로덕션 패키지 봉인

### 목적

검증된 라이브 제품과 내부 프로덕션 패키지가 같은 제품임을 최종 고정한다.
...
> GPAO-T 내부 프로덕션 패키지 검증 공정이 봉인되었다. 동일한 canonical 배포본이 독립 GPAO-T 서비스로 실제 라이브되어 있으며, 설치·상태 이전·대화·멀티 세션·Telegram 경계·Safari 데스크톱/모바일·rollback·재설치·전체 테스트·소스 봉인을 모두 통과했다.
...
이 항목들은 GPAO-T 내부 프로덕션 패키지 검증 이후 별도 승인 공정으로 다룬다.
