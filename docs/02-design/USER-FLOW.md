...
The current major axis is Model Router replay/policy. The user should be able to inspect which model path GPAO-T would choose, why that path is speed/quality/cost/risk appropriate, what fallback and failure states exist, when a Context Mesh task packet can become a model input candidate, and why model output cannot directly trigger tool/CLI/MCP execution. Live submission, provider calls, tool execution, connector activation, approval writes, install/update/rollback, and durable memory promotion remain blocked. The next major axis after this is connector/tool governance.
