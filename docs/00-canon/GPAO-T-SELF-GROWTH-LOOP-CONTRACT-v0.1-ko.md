...
GPAO-T는 독립적인 로컬 우선 Growth Personal AI Operating System이다. 사용자의 성장을 돕는 동시에 자기 자신도 더 나아지는 운영체제가 되어야 한다.
...
- live GPAO-T runtime mutation involving credentials, external effects, or irreversible state
...
## 7. 독립 런타임과 호환성 계층의 관계

GPAO-T의 자가성장 루프는 독립 제품 계약을 따른다. 비교 제품이나
호환성 계층에서 발견한 문제는 증거가 될 수 있지만, 그 코드나 상태가
GPAO-T의 기억 권한을 자동으로 갖지는 않는다.

호환성 런타임에서 발견될 수 있는 문제:
...
2026-07-14 라이브 구현 증거:

- `src/core/auto-memory-growth-loop.js`는 모든 턴을 분류하되, routine turn은
  `not_growth_material`로 끝내고 review queue, T-cell candidate, approval,
  Context Mesh apply를 변경하지 않는다.
- 명시적이고 의미 있는 기억/원칙 신호만 source-linked review candidate로
  캡처할 수 있다. 이 상태는 `captured_review_only`이며 승인이나 적용을
  뜻하지 않는다.
- Context Mesh apply는 구조화된 명시적 승인과 추적 가능한 근거가 없으면
  열리지 않는다. 런타임은 승인 레코드를 만들어내거나 자동 적용했다고
  주장해서는 안 된다.
...
- `test/auto-memory-growth-loop.test.js`는 routine turn 무변경, 명시적 교정
  후보, 승인 경계, Gateway 노출, policy verification을 검증한다.
- 라이브 근거는
