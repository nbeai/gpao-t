...
GPAO-T 작업은 라이브 OpenClaw 개조 중 치명적인 한계를 만났다.

문제는 단순히 기능이 부족했던 것이 아니다. Codex/OpenClaw 기반 작업 흐름이 사용자의 입장에서 원하는 작동, 기능, 체감, 흐름을 충분히 이해하지 못했고, 그 결과 개발 방향이 너무 구현 중심으로 꼬였다.

따라서 이 문서는 GPAO-T 개발을 다음 관점으로 재정렬한다.

> GPAO-T는 MVP나 성능 낮은 OpenClaw 개조물이 아니다.  
> GPAO-T는 OpenClaw, Codex, Claude, 로컬 기반 AI 도구, 에이전트 런타임, UI/UX, 사용자 체감 성능을 총괄 연구한 뒤, 그 위에 T-cell 이론과 GPAO Core Engine을 입혀 만드는 우리만의 자가성장형 퍼스널 AI 운영체제다.

## 2. 금지되는 작업 방식

다음 방식은 GPAO-T 개발 방향에서 금지한다.

- OpenClaw를 고치는 일을 GPAO-T 개발로 착각하는 것
...
- OpenClaw: 로컬 PC 기반 실행, 도구 연결, MCP/CLI 흐름, 세션/작업 운영
...
OpenClaw, Codex, Claude, 로컬 AI 도구, 에이전트 런타임, memory OS, agent OS 사례를 조사한다.
...
### Phase D. Runtime Absorption

OpenClaw live body와 순정 OpenClaw lab source를 GPAO-T의 런타임 몸체로 흡수한다.
...
현재 GPAO-T의 Runtime / Substrate Lab은 다음으로 제한한다.

- live OpenClaw body on 윤's PC
- pure OpenClaw source anatomy under `openclaw-clean-lab/github-openclaw-source`
- GPAO-T dashboard lab under `openclaw-clean-lab/gpao-t-openclaw-dashboard-lab`
- GPAO-T source, docs, tests, and evidence under this repository

`gpao-t-hardware-engine`은 현재 공정에서 제외한다. 이 저장소는 GPAO-T test-team candidate, live OpenClaw absorption, source seal, package seal, or completion gate가 아니다.
...
2. OpenClaw / Codex / Claude / local AI tools를 사용자 체감 기준으로 비교한다.
3. 그 결과를 바탕으로 GPAO-T의 first real OS flow를 다시 정의한다.

그 뒤에는 live OpenClaw prompt-path check, UI surface redesign, memory/replay/self-growth hardening을 이어간다.
...
- 왜 OpenClaw나 Codex나 Claude만 쓰면 안 되는가?
