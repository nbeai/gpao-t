...
Date: 2026-07-10  
Owner: 윤 (@aigis0927)  
Scope: 일반 사용자인 윤이 Codex와 함께 GPAO-T를 개발하고, 로컬 PC의 OpenClaw를 GPAO-T로 흡수해 가는 작업 방식
...
- OpenClaw live 변경은 OpenClaw 자체 개선이 아니라 GPAO-T OS를 만들기 위한 substrate 흡수 작업이다.

## 2. 우리가 만들고 있는 것

GPAO-T는 자가성장형 퍼스널 AI 운영체제다.

한 문장으로 정의하면:

> GPAO-T는 OpenClaw의 로컬 PC 운영 능력 위에 T-cell 이론과 GPAO Core Engine을 결합해, 일반 사용자가 어렵지 않게 AI-native / AX 방식으로 일하고 배우고 성장할 수 있게 돕는 자가성장형 퍼스널 AI OS다.
...
- 사용감이 불편하거나 불안하지 않은가
- GPAO-T답게 성장하고 있는가
- OpenClaw 기능을 흡수하되 GPAO-T의 주도권을 잃지 않았는가
...
## 5. OpenClaw를 GPAO-T로 흡수하는 방식

OpenClaw는 GPAO-T의 몸체가 될 수 있다.

하지만 OpenClaw가 GPAO-T의 뇌가 되면 안 된다.

OpenClaw를 live 상태에서 손대는 이유는 OpenClaw를 더 좋은 제품으로 만들기 위해서가 아니다.

그 이유는 GPAO-T라는 우리만의 자가성장형 퍼스널 AI OS를 만들기 위해, OpenClaw의 로컬 PC 운영 능력을 흡수하기 위해서다.
...
```text
OpenClaw 기능은 흡수한다.
OpenClaw UX와 의미론은 그대로 따르지 않는다.
GPAO-T의 기억, 권한, 성장, 말귀 구조가 우선이다.
OpenClaw 작업이 GPAO-T 전체 설계에서 벗어나면 즉시 멈추고 재정렬한다.
```
...
우리가 OpenClaw 자체 개선으로 편협해지고 있지는 않은가?
```
...
특히 OpenClaw live 경로는 실제 live turn으로 확인되기 전까지 성공이라고 말하지 않는다.
...
- 문서 없이 감으로 진행하지 않는다.
- 리서치가 필요하면 즉시 리서치한다.
- live OpenClaw 변경 전에는 작업명령과 롤백 경계를 만든다.
- 일반 사용자의 체감을 항상 기준에 둔다.
- GPAO-T가 자가성장하더라도 몰래 바뀌지 않게 한다.
- OpenClaw를 흡수하되, GPAO-T의 존재 이유를 흐리지 않는다.
