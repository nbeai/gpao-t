...
리서치, 이론 정리, OpenClaw live 경로 분석, 구현, 테스트, UX 검토, 보안/권한 검토, GitHub 완료 프로젝트 참조가 동시에 필요할 수 있다.
...
- OpenClaw live 경로와 GPAO-T 제품 설계를 분리해서 검토해야 할 때
- 구현자와 리뷰어를 분리해야 할 때
- 테스트/QA 관점이 따로 필요할 때
...
- OpenClaw live 경로 분석과 GPAO-T 제품 설계를 분리해서 봐야 하는 작업
...
에이전트 산출물은 기본적으로 review candidate다. 바로 제품 원칙, Canon, runtime, durable memory, live OpenClaw, public release로 승격하지 않는다.
...
| 역할 | 임무 | 산출물 |
| --- | --- | --- |
| Flow Keeper | 전체 작업이 OpenClaw 흡수 경로, 권한 경계, 증거 조건, 다음 행동 압축에서 벗어나지 않는지 감시 | drift / blocker / evidence / authority / next action check |
| Research Agent | GitHub, 논문, 커뮤니티, nbeai 완료 프로젝트 조사 | source-grounded research note |
| Architecture Agent | GPAO-T 전체 OS 구조와 OpenClaw 흡수 경계 검토 | architecture decision memo |
| Runtime Agent | OpenClaw live 경로, prompt path, gateway, session, model/tool route 분석 | runtime evidence / patch proposal |
...
- `Operating Surface`: live OpenClaw dashboard absorption, multi-session/work panes, chat UX, inspector, visible memory/replay/apply/rollback state, desktop/mobile QA.
...
GPAO-T 작업에서 과거 완료 프로젝트, release, package, plugin, adapter, harness, Knowledge Loop, OpenClaw 관련 근거가 필요하면 nbeai GitHub를 중요한 외부 source-of-truth로 참고한다.
...
- OpenClaw 흡수 작업을 OpenClaw 개선 프로젝트로 오해하는 것
- public release 상태와 local/live 후보 상태를 섞는 것
...
- Codex-side GPAO가 OpenClaw live 경로를 자동으로 고쳤다는 뜻은 아니다.
- 플러그인 설치 사실만으로 live behavior proof가 생기는 것은 아니다.

## 6. OpenClaw 작업의 목적 경계

OpenClaw live 변경은 OpenClaw 개선이 목적이 아니다.

목적은 GPAO-T OS를 만들기 위해 OpenClaw의 로컬 PC 운영 능력을 흡수하는 것이다.

따라서 모든 OpenClaw 관련 작업은 다음 질문을 통과해야 한다.

```text
이 변경은 GPAO-T의 OS 완성도를 높이는가?
이 변경은 GPAO-T의 기억, 맥락, 권한, 성장, 사용자 체감을 증명하는가?
이 변경이 OpenClaw 자체 최적화로 좁아지고 있지는 않은가?
```
