...
- `gpao-t owner-ops workflow <review_reply|shopping_inquiry|reservation_inquiry>`
- `gpao-t owner-ops record <workflow> <text>`
...
v0.1.3부터 GPAO-T는 `skill execute <text>`로 실행 계획을 실제 로컬 실행 증거로 변환한다.

```text
skill execution plan
-> local artifact drafts
-> quality gate results
-> replay evidence
-> growth signal candidates
-> optional local history record
```

이 어댑터는 외부 도구를 실행하는 기능이 아니다. 목적은 선택된 스킬팩이 실제 작업 품질로 내려왔는지 확인 가능한 증거를 만드는 것이다.

예를 들어 `디자인 좋은 웹앱을 만들어줘`라는 요청은 다음 식으로 변환된다.
...
각 산출물은 로컬 초안 상태이며, 완료 주장은 품질 게이트 결과를 통과해야 한다. 반복 실패나 review 신호는 `growthSignalCandidates`로 남아 나중에 스킬 업그레이드 제안의 재료가 된다.

기록형 명령 `skill execute-record <text>`는 `.gpao-t/skill-execution/history.jsonl`에 로컬 실행 증거를 남긴다. 이것은 durable memory promotion이 아니며, live skill mutation도 아니다.
...
4. 한국 사업 / Owner Ops 스킬팩의 no-API workflow와 MCP/source registry 연결
