...
- public release prerequisite, internal production review packet, internal production owner decision lane, internal production next-action packet은 빠르게 읽히지만, release/upload authority는 닫혀 있는가
...
- `internal production review packet`: production package, release readback, next owner testing loop, authority gate, decision record를 묶은 owner 검토 표면
- `internal production owner decision lane`: owner가 local-only 결정을 남길 수 있는 token-gated 표면
- `internal production next-action packet`: owner decision 값을 다음 local operating surface로 연결하는 read-only 안내 표면
