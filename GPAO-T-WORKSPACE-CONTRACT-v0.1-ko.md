...
이 문서는 GPAO-T를 단일 구현 폴더나 OpenClaw 플러그인으로 좁히지 않기 위한 작업 환경 계약이다.

GPAO-T는 채널 독립적인 Growth Personal AI Operating System이다. OpenClaw 계열 런타임은 안정적인 기계 몸체로 흡수할 수 있지만, GPAO-T의 운영 의미, Context / T-cell Kernel, 권한 의미론, 모델 라우터, 커넥터 거버넌스, 성장 루프, 사용자 경험은 GPAO-T가 소유한다.

따라서 GPAO-T 작업 환경은 다음 네 레인을 분리한다.

```text
1. Product / Kernel Source of Truth
2. Runtime / Substrate Lab
...
GPAO-T는 메신저 봇, OpenClaw 플러그인, 관리 대시보드, 단일 Codex skill pack이 아니다.
...
### 2.2 OpenClaw는 substrate다

OpenClaw 계열 런타임은 재사용 가능한 기계 몸체다.
...
- OpenClaw-like substrate absorption
- live-safe runtime patch planning
- gateway/session/tool skeleton checks
- OpenClaw turn-start input packet attachment checks
...
- substrate가 GPAO-T의 의미론을 정의하지 않는다.
...
```text
1. controlled OpenClaw turn smoke
...
- OpenClaw Control UI placement analysis
...
```text
/Users/jyp/Documents/Playground 2/gpao-t/.beai-harness
...
- internal candidate is not live behavior-confirmed unless live behavior was actually verified.
...
실제 OpenClaw-like runtime, gateway, session, prompt path, service lifecycle을 바꾸는가?
-> openclaw-clean-lab / live OpenClaw patch evidence under gpao-t
...
- OpenClaw substrate work를 GPAO-T product truth로 착각
- Gateway Control UI를 최종 GPAO-T UX로 착각
- Context Mesh 후보를 durable memory로 착각
- replay artifact를 live behavior proof로 착각
- internal candidate를 public release로 착각

### P0. Live-safe OpenClaw turn smoke

GPAO-T OpenClaw conversion Stage 1의 다음 큰 작업은 UI 재설계가 아니다.

다음이 먼저다.

```text
controlled OpenClaw turn
...
```text
1. GPAO-T live-safe OpenClaw turn smoke plan
