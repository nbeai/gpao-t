...
const SECRET_VALUE = /(?:^|\s)(?:sk-[A-Za-z0-9_-]{12,}|gpao(?:n|t3)_[A-Za-z0-9_-]{20,}|Bearer\s+[A-Za-z0-9._-]{12,}|AKIA[0-9A-Z]{16})(?:$|\s)/;
...
export function assertCapabilityPayloadHasNoSecrets(payload) {
  const location = inspect(payload, "capabilityPayload");
  if (location) throw new RuntimeError("secret_in_capability_payload", "Capability data must use a protected secret reference", 400, { field: location });
}
