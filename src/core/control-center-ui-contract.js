...
const VISUAL_SECTIONS = [
  ...
  {
    id: "workflow_state_view",
    label: "Workflow State View",
    objectType: "Evidence",
    requiredFields: ["workflowState", "recoveryState", "authorityState", "nextActionState"],
    sourceFields: ["snapshot.status", "snapshot.counts", "snapshot.authorityBoundary", "snapshot.nextSafeAction"],
  },
  {
    id: "panel_grid",
    label: "Panel Grid",
    objectType: "Work",
    requiredFields: ["panelId", "group", "label", "status", "headline", "nextSafeAction", "stateLens"],
...
export function buildControlCenterUiContract() {
  return {
    ...
    validationRules: [
      ...
      "State/workflow views must be derived from the snapshot and must not claim live execution authority.",
