...
function canonical(permit) {
  return JSON.stringify({ invocationId: permit.invocationId, commandId: permit.commandId, taskPacketId: permit.taskPacketId, principalId: permit.principalId, generation: permit.generation, toolId: permit.toolId, action: permit.action, argsDigest: permit.argsDigest, effect: permit.effect, approvalId: permit.approvalId || null, idempotencyKey: permit.idempotencyKey, expiresAt: permit.expiresAt });
}
export function createToolCall({ taskPacketId, commandId, principalId, generation, toolId, action, args, idempotencyKey }) {
  ...
  return { schema: "gpao_t.tool_call.v1", invocationId: `tool_${crypto.randomUUID()}`, taskPacketId, commandId, principalId, generation, toolId, action, argsDigest: digest(args), idempotencyKey };
}
