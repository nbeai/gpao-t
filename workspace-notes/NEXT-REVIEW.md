...
Add the next smallest user-visible read-only refinement to `/work-surface` only if it improves task understanding. Keep live submission and model/tool/connector execution blocked.

## Review Before Continuing

- Preserve the `/work-surface` visual QA baseline: nonblank viewport, draft input visibility, native readability details, task state visibility, context/skill route readability, authority boundary visibility, next safe action visibility, no overflow, mobile topbar/action visibility, no script, no form, and no external activation.
...
Resume from the read-only GPAO-T core work surface. Do not reopen deeper Tauri/dry-run/approval meta-gates unless the user explicitly asks for a concrete mutating action gate.
...
- visual QA: `/work-surface` desktop 1440x960 and mobile 390x844 screenshots refreshed with native readability details.
